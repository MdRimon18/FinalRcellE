...
CREATE PROCEDURE [dbo].[Locations_InsertOrUpdate_SP](
  @LocationId bigint,
  ...
  @SuccessOrFailId INT OUTPUT
      ...
      SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED;
		...
		IF EXISTS (
			...
			Status='Active' and
			@LocationId=0 and
			LOWER(LTRIM(RTRIM(REPLACE([LocationName], ' ', '')))) = LOWER(LTRIM(RTRIM(REPLACE(@LocationName, ' ', ''))))
		)
		BEGIN 
         ...
         Else
		   BEGIN TRY  --try start for update
					...
					SET 
					 ...
					 FROM 
					   [stt].[Locations] a WITH(ROWLOCK) 
					  WHERE 
					  a.LocationId = @LocationId 

					 SET @SuccessOrFailId = @LocationId; -- Set success flag  updated Id
				 END TRY
				 BEGIN CATCH
