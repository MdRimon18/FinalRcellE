

Create PROCEDURE [dbo].[PromoOrCupponCode_InsertOrUpdate_SP](
  @PromoOrCuppnId bigint,
  @PromoOrCuppnKey uniqueidentifier= NULl,
  @PromoOrCuppnName nvarchar(300),  
  @Code varchar(20),
  @Discount decimal(5, 2) ,
  @ValidFrom datetime,
  @ValidTo datetime,
  @MaxUses int =NULL,
  @RemainingUses int =NULL,
  @Description nvarchar(max)= NULL,
  @EntryDateTime DATETIME=NUll,
  @EntryBy BIGINT=NULL,
  @LastModifyDate DATETIME = NULL,
  @LastModifyBy BIGINT = NULL,
  @DeletedDate DATETIME = NULL,
  @DeletedBy BIGINT = NULL,
  @Status VARCHAR(10)=NuLL,
  @SuccessOrFailId INT OUTPUT
) AS BEGIN 
      SET NOCOUNT ON;
      SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED;

		-- Check if @unitName already exists (case-insensitive, trimmed, and with white spaces replaced)
		IF EXISTS (
			SELECT 1
			FROM  [stt].[PromoOrCuppnCode] 
			WHERE
			Status='Active' and
			@PromoOrCuppnId=0 and
			LOWER(LTRIM(RTRIM(REPLACE([PromoOrCuppnName], ' ', '')))) = LOWER(LTRIM(RTRIM(REPLACE(@PromoOrCuppnName, ' ', ''))))
		)
		BEGIN 
			-- If BillingPlanName already exists, set  @BillingPlanId to -1
			SET @SuccessOrFailId =-1;
		END
       ELSE
         BEGIN
		  IF(@PromoOrCuppnId=0)
			 Begin 
			  -- Insert new record
			  INSERT INTO  [stt].[PromoOrCuppnCode] (
				   [PromoOrCuppnKey]
                  ,[PromoOrCuppnName]
                  ,[Code]
                  ,[Discount]
                  ,[ValidFrom]
                  ,[ValidTo]
                  ,[MaxUses]
                  ,[RemainingUses]
                  ,[Description]			
				  ,[EntryDateTime]
				  ,[EntryBy]
				  ,[LastModifyDate]
				  ,[LastModifyBy]
				  ,[DeletedDate]
				  ,[DeletedBy]
				  ,[Status]
			   ) 
		VALUES (
				NEWID(),  
				
				@PromoOrCuppnName,
				@Code,
				@Discount,
				@ValidFrom,
				@ValidTo,
				@MaxUses,
				@RemainingUses,
				@Description,
				@EntryDateTime,
				@EntryBy,
				@LastModifyDate,
				@LastModifyBy,
				@DeletedDate,
				@DeletedBy,
				'Active'
			);
			   SET @SuccessOrFailId = SCOPE_IDENTITY(); --Get Newly Created Id
             --  SELECT  @BillingPlanId AS BillingPlanId;  --Set Newly Created Id
			 
			  End
         Else
		   BEGIN TRY  --try start for update
					UPDATE 
					a WITH(ROWLOCK) 
					SET 
					  
					   
					  [PromoOrCuppnName] = @PromoOrCuppnName,
					  [Code] = @Code,
					  [Discount] = @Discount,
					  [ValidFrom] = @ValidFrom,
					  [ValidTo] = @ValidTo,
					  [MaxUses] = @MaxUses,	
					  [RemainingUses] = @RemainingUses,	
					  [Description] = @Description,	
					 
					  [LastModifyDate] = COALESCE(@lastModifyDate, [LastModifyDate]),
					  [LastModifyBy] = COALESCE(@lastModifyBy, [LastModifyBy]),
					  [DeletedDate] = COALESCE(@deletedDate, [DeletedDate]),
					  [DeletedBy] = COALESCE(@deletedBy, [DeletedBy]),
					  [Status] = COALESCE(@status, [Status])
					 FROM 
					   [stt].[PromoOrCuppnCode] a WITH(ROWLOCK) 
					  WHERE 
					  a.PromoOrCuppnId = @PromoOrCuppnId 

					 SET @SuccessOrFailId = @PromoOrCuppnId; -- Set success flag  updated Id
				 END TRY
				 BEGIN CATCH
					SET @SuccessOrFailId = 0; -- Error occurred, set success flag to 0
				 END CATCH
		 END 

		  
END;

--select * from [stt].[BillingPlans]